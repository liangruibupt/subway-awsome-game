...
import type { TrainHead, Carriage } from '../../types';
...
const ERA_ICON: Record<Era, string> = {
  classic: '[C]',
  modern: '[M]',
  future: '[F]',
};
...
export function TrainCatalog() {
  ...
  const headsForEra = trainCatalog.filter(
    (item) => item.kind === 'head' && item.era === selectedEra
  );

  const carriageItems = trainCatalog.filter((item) => item.kind === 'carriage');
  ...
  function handleCarriageClick(item: CatalogItem) {
    ...
    addCarriage(activeTrain.id, carriage);
  }
  ...
  function catalogCard(
    ...
    return (
      ...
      >
        ...
        <div
          style={{ fontSize: 8, color: '#b2bec3', marginTop: 2, lineHeight: 1.3 }}
        >
          {item.description}
        </div>
      </button>
    ...
    >
            ...
            style={{
              ...
              border:
                selectedEra === era
                  ...
                  : '1px solid #1a3a5c',
              background:
                selectedEra === era ? '#2d1b6988' : 'transparent',
              color: selectedEra === era ? '#a29bfe' : '#b2bec3',
              borderRadius: 3,
              transition: 'all 0.15s',
            }}
            title={`${ERA_ICON[era]} ${label} era trains`}
          >
            {label}
          </button>
        ))}
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {headsForEra.map((item) =>
            catalogCard(item, () => handleHeadClick(item))
          )}
        </div>
      </div>

      {/* Divider */}
      <div style={{ borderBottom: '1px solid #1a3a5c44' }} />
      ...
      <div>
        ...
        {!activeTrain && (
          ...
          >
            Pick a train head first!
          </div>
        )}
        {atMaxCarriages && activeTrain && (
          <div
            style={{
              fontSize: 9,
              color: '#ffd93d',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {carriageItems.map((item) => {
            const disabled = !activeTrain || atMaxCarriages;
            const xlBadge =
